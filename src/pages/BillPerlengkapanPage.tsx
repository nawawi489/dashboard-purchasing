...
import { submitPerlengkapanPaymentProof } from '../services/perlengkapanApproval'
import { ApprovalItem, InventoryApprovalItem } from '../types'
import { OUTLETS } from '../constants'
import { usePerlengkapanApprovalItemsWithOutletFilter } from '../hooks/usePerlengkapanApprovalItemsWithOutletFilter'
...
export default function BillPerlengkapanPage() {
  const {
    ...
    setOutletFilter,
    loadData,
  } = usePerlengkapanApprovalItemsWithOutletFilter('Tidak dapat memuat data tagihan perlengkapan')
  ...
  const handleSubmitPaymentProof = async (trxId: string, file: File, invoiceNumber: string) => {
    ...
    try {
      const success = await submitPerlengkapanPaymentProof(trxId, file, modalState.outlet, invoiceNumber)
      if (success) {
