...
import {
  WEBHOOK_APPROVE_PENGAJUAN_PERLENGKAPAN,
  WEBHOOK_LIST_PENGAJUAN_PERLENGKAPAN,
  WEBHOOK_LIST_PENGAJUAN_PERLENGKAPAN_APPROVED,
  WEBHOOK_PAYMENT_PROOF_PERLENGKAPAN,
} from '../config'
import {
...
export { type InventoryApprovalItem as PerlengkapanApprovalItem }
...
async function fetchPerlengkapanItemsFrom(
  baseUrl: string,
  outlet?: string,
  ...
  try {
    ...
    return list.map((row: any): InventoryApprovalItem => {
      // Sheet dipisah per-outlet (di-switch oleh n8n lewat query "outlet"),
      // jadi baris tidak selalu membawa kolom Outlet sendiri — pakai outlet yang diminta.
      const rowOutlet =
        row.outlet ||
        row.Outlet ||
        row.OUTLET ||
      ...
      return {
        ...
        date: normalizeText(
          row['Tanggal Pengajuan'] || row['tanggal pengajuan'] || row['Tanggal Permintaan'] || row.date || '',
        ),
        ...
        unit: normalizeText(row['Satuan Barang'] || row['satuan barang'] || row.Satuan || row.unit || ''),
        quantity: normalizeNumber(row.Qty ?? row.qty ?? row['Total QTY'] ?? row.quantity ?? 0),
...
export async function fetchPerlengkapanApprovalItems(
  ...
  return fetchPerlengkapanItemsFrom(WEBHOOK_LIST_PENGAJUAN_PERLENGKAPAN, outlet)
}

export async function fetchApprovedPerlengkapanItems(
  outlet?: string,
): Promise<InventoryApprovalItem[]> {
  return fetchPerlengkapanItemsFrom(WEBHOOK_LIST_PENGAJUAN_PERLENGKAPAN_APPROVED, outlet)
}
